...
Un contenedor contiene (valga la redundancia) una secuencia de
objetos. Si no tuvieramos contenedores, tendríamos que recordar el
lugar de cada uno para cada vez que necesitamos usarlo. En cambio,
podemos guardarlos todos juntos y sólo recordar el lugar donde está la
colección. Esto implica, sin embargo, que entonces haya que tener que
buscarlo dentro de ese contenedor cuando lo necesitemod. Pero eso es
mejor a tener que anotar dónde está cada cosa. Perderíamos mucho
tiempo pensando qué nombres poner a las variables, y los de muchas de
ellas serían necesariamente malos, o estarían numeradas por ejemplo,
etc.
...
global. Pero en R también podemos hacer eso:

#+begin_src R
mean = Reduce("+",x)
#+end_src

Estas dos funciones, ~Reduce~ y ~srd::accumulate~ son bastante
parecidas. La idea es que tenemos una colección, una operación binaria
uno de cuyos parámetros será del tipo de los elementos de la
colección, y un valor inicial, que también es parámetro de la función,
y es del mismo tipo que su resultado. En este caso particular la
operación es la suma, o sea que los parámetros son ambos del mismo
tipo. En ~Reduce~ el valor inicial está implícito, y es el cero, pero
podemos pasarle otro valor. La diferencia más importante es la forma
en que se indica la colección. En c++ le decimos no la colección sino
un rango dentro de la colección (en este ejemplo es el rango con todos
los elementos). Pero esto es bueno, pero entonces puede usarse para
casos en los que sólo queremos aplicarle una función a una parte. 

Per veamos ahora qué contenedores hay en C++ y la stl.


El tipo más simple es un arreglo (un /built-in array/), que se define,
...
La verdad que este tipo no suele ser la mejor opción nunca. un
problema que puede aparecer con el array es que no sabemos cuantos
elementos contiene. Tenemos que memorizarlo nosotros, y pasarselo a la
función que lo vaya a usar. Esto en sí no es muy grave, creo. Pero lo
que sí es que si por algún error hacemos ~x[M]~, donde ~M~ es mayor al
tamaño del arreglo, entonces obtenemos una conducta idefinida.
