...
Los contenedores sirven para guardar secuencias de objetos. Si no
...
Estas dos funciones, ~Reduce~ y ~std::accumulate~ son bastante
...
#+begin_src c++
template<size_t N>
bool is_local_extremum(const int (&x)[N], size_t n);
#+end_src
...
bool is_local_extremum(const T (&x)[N], size_t n);
#+end_src
...
excepción ~std::out_of_range~ si la posión pedida está fuera del
...
probablemente mas usado es el ~std::vector~:
...
contenedores de este tipo se incluyen ~std::list~ (double linked) y
~std::forward_list~ (singly-linked), ~std::deque~, ~std::set~ y
~std::map~.
